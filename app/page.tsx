...
const EDUCATION_DRUGS: Record<EducationMode, string[]> = {
  ...
  colonoscopy: ["수클리어산", "오라팡정", "원프렙 1.38산", "플렌뷰산", "피코솔루션액", "하프렙산"].sort((a, b) => a.localeCompare(b, "ko")),
};
...
const PATIENT_EDUCATION_PDFS: Record<EducationMode, PatientEducationItem[]> = {
  ...
  colonoscopy: [
    { id: "colonoscopy-suclear", drugName: "수클리어산", fileName: "수클리어산.pdf", pdfData: "/patient-education/colonoscopy/수클리어산.pdf" },
    { id: "colonoscopy-orapang", drugName: "오라팡정", fileName: "오라팡정.pdf", pdfData: "/patient-education/colonoscopy/오라팡정.pdf" },
    { id: "colonoscopy-oneprep", drugName: "원프렙 1.38산", fileName: "원프렙 1.38산.pdf", pdfData: "/patient-education/colonoscopy/원프렙 1.38산.pdf" },
    { id: "colonoscopy-plenvu", drugName: "플렌뷰산", fileName: "플렌뷰산.pdf", pdfData: "/patient-education/colonoscopy/플렌뷰산.pdf" },
    { id: "colonoscopy-picosolution", drugName: "피코솔루션액", fileName: "피코솔루션액.pdf", pdfData: "/patient-education/colonoscopy/피코솔루션액.pdf" },
    { id: "colonoscopy-halfrep", drugName: "하프렙산", fileName: "하프렙산.pdf", pdfData: "/patient-education/colonoscopy/하프렙산.pdf" },
  ].sort((a, b) => a.drugName.localeCompare(b.drugName, "ko")),
};
...
const getErDiscardDate = (baseDate = new Date(), months = 6): string => {
  const d = new Date(baseDate);
...
const getErExpiryMonths = (drugName: string): 1 | 6 => ER_ONE_MONTH_DRUGS.includes(drugName.trim()) ? 1 : 6;

const getErDrugNameLines = (name: string): string[] => {
...
export default function App() {
  ...
  const [selectedSyrupLabels, setSelectedSyrupLabels] = useState<string[]>([]);
  const [erLabelDrug, setErLabelDrug] = useState(ER_LABEL_PRESETS[0] ?? "");
  ...
  const syrupLabelPrintList = selectedSyrupLabels.length > 0 ? selectedSyrupLabels : [syrupLabelName];
  ...
  const addSyrupLabel = () => {
    const name = syrupLabelName.trim();
    if (!name) return;
    setSelectedSyrupLabels((prev) => (prev.includes(name) ? prev : [...prev, name]));
    setSyrupLabelCustomDrug("");
  };
  const removeSyrupLabelAt = (index: number) => setSelectedSyrupLabels((prev) => prev.filter((_, i) => i !== index));
  const clearSelectedSyrupLabels = () => setSelectedSyrupLabels([]);
  const addErLabel = () => {
  ...
  const handleSyrupLabelPrint = () => {
    const printWindow = window.open("", "_blank", "width=900,height=700");
    if (!printWindow) return;
    const sheets = syrupLabelPrintList.map((name) => {
      const columns = getSyrupColumnCount(name);
      const total = columns * syrupLabelRows;
      const text = `${name} ${syrupLabelExpiry}`;
      const cells = Array.from({ length: total }, () => `<div class="syrup-label-cell">${text}</div>`).join("");
      return `<div class="syrup-label-sheet" style="grid-template-columns: repeat(${columns}, max-content); column-gap: 1.8mm;">${cells}</div>`;
    }).join("");
    printWindow.document.write(`<!doctype html><html lang="ko"><head><meta charset="utf-8" /><title>시럽 라벨 출력</title><style>@page { size: 90mm 100mm; margin: 0; } html, body { margin: 0; padding: 0; background: white; font-family: "Malgun Gothic", Arial, sans-serif; } body { -webkit-print-color-adjust: exact; print-color-adjust: exact; } .syrup-label-sheet { width: 90mm; height: 100mm; box-sizing: border-box; padding-left: 3mm; padding-top: 2mm; display: grid; grid-template-rows: repeat(21, 1fr); column-gap: 1.8mm; row-gap: 0; overflow: hidden; background: white; page-break-after: always; } .syrup-label-cell { display: flex; align-items: center; justify-content: flex-start; text-align: left; font-size: 8pt; line-height: 1; white-space: nowrap; box-sizing: border-box; padding-left: 3mm; font-weight: 900; }</style></head><body>${sheets}</body></html>`);
    printWindow.document.close();
    printWindow.focus();
    printWindow.print();
  };

  const handleErLabelPrint = () => {
    const printWindow = window.open("", "_blank", "width=900,height=700");
    if (!printWindow) return;
    const list = selectedErLabels.length > 0 ? selectedErLabels : erLabelPreviewList;
    const repeated = Array.from({ length: erLabelCopies }, () => list).flat();
    const sheets: string[] = [];
    repeated.forEach((item) => {
      const group = Array.from({ length: 4 }, () => item);
    ...
    const html = "<!doctype html><html lang=\"ko\"><head><meta charset=\"utf-8\" /><title>응급실 라벨 출력</title><style>@page { size: 90mm 50mm; margin: 0; } html, body { margin: 0; padding: 0; background: white; font-family: \"Malgun Gothic\", Arial, sans-serif; } body { -webkit-print-color-adjust: exact; print-color-adjust: exact; } .er-label-sheet { width: 90mm; height: 50mm; box-sizing: border-box; padding-top: 6mm; padding-left: 7mm; background: white; page-break-after: always; overflow: hidden; } .er-label-grid { display: grid; grid-template-columns: repeat(4, 19mm); grid-template-rows: 29mm; width: 76mm; height: 29mm; border-top: 0.35mm solid #000; border-left: 0.35mm solid #000; } .er-label-cell { width: 19mm; height: 29mm; box-sizing: border-box; border-right: 0.35mm solid #000; border-bottom: 0.35mm solid #000; overflow: hidden; padding: 1.1mm 1mm 0.7mm 1mm; text-align: center; color: #000; font-weight: 700; } .er-drug-name { height: 9.2mm; display: flex; flex-direction: column; align-items: center; justify-content: center; font-size: 3.7mm; line-height: 0.95; letter-spacing: -0.25mm; word-break: keep-all; } .er-volume { height: 5.2mm; display: flex; align-items: center; justify-content: center; font-size: 3.9mm; line-height: 1; font-weight: 800; } .er-date-block { margin-top: 1.2mm; text-align: left; font-size: 2.35mm; line-height: 1.18; font-weight: 600; letter-spacing: -0.1mm; }</style></head><body>" + sheets.join("") + "</body></html>";
  ...
  <div className="space-y-5">
    <div className="grid gap-4 md:grid-cols-3">
      ...
      <div className="flex items-end gap-2">
        <PebbleButton onClick={addSyrupLabel} variant="sage" className="h-10 px-5 py-0 text-sm flex items-center justify-center leading-none">목록 추가</PebbleButton>
        <PebbleButton onClick={() => setSyrupLabelCustomDrug("")} variant="light" className="h-10 px-4 py-0 text-sm flex items-center justify-center leading-none">직접입력 초기화</PebbleButton>
      </div>
    </div>

    <div>
      <div className="mb-2 flex items-center justify-between">
        <Label>선택된 시럽 라벨 목록</Label>
        {selectedSyrupLabels.length > 0 && <button type="button" onClick={clearSelectedSyrupLabels} className="text-sm text-[#8a8175] underline underline-offset-2">전체 비우기</button>}
      </div>
      <div className="rounded-2xl border border-input bg-background p-4">
        {selectedSyrupLabels.length > 0 ? <div className="grid gap-2 grid-cols-2 md:grid-cols-4 xl:grid-cols-6">{selectedSyrupLabels.map((name, index) => <div key={`${name}-${index}`} className="rounded-xl border border-[#e5dccf] bg-white px-3 py-2 text-sm text-[#5f574d]"><div className="font-semibold text-[#3e372f]">{name}</div><div className="mt-1 text-xs text-[#8a8175]">{name} {syrupLabelExpiry}</div><div className="mt-2 flex justify-between text-xs text-[#8a8175]"><span>{index + 1}번 라벨</span><button type="button" onClick={() => removeSyrupLabelAt(index)} className="underline underline-offset-2">제거</button></div></div>)}</div> : <div className="flex min-h-[80px] items-center justify-center text-sm text-[#8a8175]">약품을 선택한 뒤 목록 추가를 누르세요. 선택 목록이 없으면 현재 약품 1개만 출력됩니다.</div>}
      </div>
    </div>
    ...
    <div className="grid gap-4 md:grid-cols-4">
      <div>
        ...
        <select value={erLabelDrug} onChange={(e) => setErLabelDrug(e.target.value)} className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm">
          {ER_LABEL_PRESETS.map((name) => <option key={name} value={name}>{name}</option>)}
        </select>
      </div>
      <div>
        <Label>직접 입력</Label>
      ...
      <div>
        <Label>출력 세트 수</Label>
        <Input value={erLabelSetCopies} onChange={(e) => setErLabelSetCopies(e.target.value.replace(/[^0-9]/g, ""))} placeholder="1" />
      </div>
    </div>
