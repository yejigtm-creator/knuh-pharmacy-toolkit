...
export default function DrugInfo() {
  const [category, setCategory] = useState<"steroid" | "antimicrobial" | "insulin">("steroid");
  return (
    <div className="min-w-0 space-y-6">
      <nav aria-label="약품정보 분류" className="flex flex-wrap gap-3">
        {([{ key: "steroid", label: "스테로이드 외용제" }, { key: "antimicrobial", label: "항감염제" }, { key: "insulin", label: "인슐린" }] as const).map(item => (
          <PebbleButton
            key={item.key}
